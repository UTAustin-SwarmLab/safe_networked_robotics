...
class RandTdContinuousCruiseCtrlEnv(gym.Env):
	...
	def __init__(self, time_delay=0, train=True, log_file_str=None, delta=1.0): 
		...
		### For random time delay purposes
		self.max_td = time_delay 
		self.state_queue = deque(maxlen=self.max_td)
		self.td_dist = [1.0, 0.0, 0.0]
	...
	def ConvertToAbstractState(self, mdp_state):
		discrete_physical_state = (max(self.min_rel_dist_disc, min(self.max_rel_dist_disc, math.floor(mdp_state[0]))), \
									max(self.min_rel_vel_disc, min(self.max_rel_vel_disc, math.floor(mdp_state[1]+5))))
	...
	def InitializeEnvironmentVariables(self):
		...
		td_state = np.array([self.max_td])
		self.mdp_state = np.concatenate((self.mdp_state, ustate, td_state))

		l = [self.ConvertToAbstractState(self.mdp_state) for cnt in range(self.max_td)]
		self.state_queue.extend(l)
	...
	def ConvertCurrentMDPStateToDiscrete(self):
		...
		abstract_state = self.ConvertToAbstractState(self.mdp_state)

		ustate = self.mdp_state[2:-1]
	...
	def step(self, action):
		...
		action = action[0]
		rel_dist = self.mdp_state[0]
		rel_vel = self.mdp_state[1]
		ustate = self.mdp_state[2:-1]
		td_state = self.mdp_state[-1]
		...
		### State transition
		"""
		def transition_fn(self, rel_dist, rel_vel, ego_acc_list):
			for ego_acc in ego_acc_list:
				...
				rel_dist = rel_dist + rel_dist_trav 
