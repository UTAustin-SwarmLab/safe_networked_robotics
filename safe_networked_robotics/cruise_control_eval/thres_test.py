...
if __name__ == "__main__":
	...
	thres_list = [0.0, 0.5, 0.8, 0.9, 0.99]
	plotting_list = []
	...
	for thres in thres_list:
		...
		for time_delay in range(1, args.max_time_delay+1):
			print("############################################################")
			print("time delay : %d", time_delay)
			args.time_delay = time_delay
			...
			plotting_list = plotting_list + list(zip([thres]*args.num_test_episodes, \
								[time_delay]*args.num_test_episodes, \
								last_dist_rem_arr_per_td, \
								min_dist_rem_arr_per_td, \
								episode_returns_arr_per_td))


	df = pd.DataFrame (plotting_list, columns = ['threshold', 'time_delay', 'end_distance', 'min_distance', 'return'])

	fig = plt.figure()
	plot = sns.boxplot(x='time_delay', y='min_distance', data=df, order = None, hue = 'threshold')
