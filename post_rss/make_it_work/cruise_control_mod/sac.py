...
from env import ConstTdContinuousCruiseCtrlEnv
...
class SoftActorCritic():
    ...
    def __init__(self, args):
        ...
        #     print("here")
        #     self.env = ConstTdContinuousCruiseCtrlEnv(time_delay=args.max_time_delay, train=True, delta=args.delta) 
        #     self.test_env = ConstTdContinuousCruiseCtrlEnv(time_delay=args.max_time_delay, train=False, delta=args.delta)
        # else:
        #     self.env = RandTdContinuousCruiseCtrlEnv(time_delay=args.max_time_delay, train=True, delta=args.delta) 
        #     self.test_env = RandTdContinuousCruiseCtrlEnv(time_delay=args.max_time_delay, train=False, delta=args.delta) 

        if args.constant_delay: 
            print("here")
            self.env = RandTdContinuousCruiseCtrlEnv(time_delay=args.max_time_delay, train=True, constant_td=True, delta=args.delta) 
            self.test_env = RandTdContinuousCruiseCtrlEnv(time_delay=args.max_time_delay, train=False, constant_td=True, delta=args.delta)
        else:
            self.env = RandTdContinuousCruiseCtrlEnv(time_delay=args.max_time_delay, train=True, delta=args.delta) 
            self.test_env = RandTdContinuousCruiseCtrlEnv(time_delay=args.max_time_delay, train=False, delta=args.delta) 
    ...
    def test(self, policy_path, args, test_ep_random_seeds):
        ...
        for test_ep_idx in range(args.num_test_episodes):
            ...
            #pmax_values.append(np.array(pmax_values_per_episode))
