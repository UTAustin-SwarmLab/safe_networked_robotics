...
def ValueIterationForActualSafety(Vmin, Qmin, threshold, mdp, labels, policy, eps=1e-16):
    num_states = Qmin.shape[0]
    num_actions = Qmin.shape[1]
    V = np.zeros((num_states,))
...
# Vmax = ValueIterationForMinSafety(Vmin, Qmin, threshold, mdp, bad_labels)
...
abstracted_dnn_policy = np.load('constant_generated/abstracted_dnn_policy_%d_td.npy' % td, allow_pickle=True).item()
Vactual = ValueIterationForActualSafety(Vmin, Qmin, threshold, mdp, bad_labels, abstracted_dnn_policy)
