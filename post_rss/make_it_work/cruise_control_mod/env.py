...
class ConstTdContinuousCruiseCtrlEnv(gym.Env):
	...
	def __init__(self, time_delay=0, train=True, log_file_str=None, delta=1.0): 
		...
		self.action_space = gym.spaces.Box(low=-5.0, high=1.0, shape=(1,))
		...
		"""
		self.td = time_delay
		shield_path = 'constant_generated/%d_td/shield_%s_prob.npy' % (self.td, str(delta))  
		...
		self.ustates = list(itertools.product(list(range(len(self.ego_acc_values))), repeat=self.td))
		self.num_ustates = len(self.ustates)
	...
	def AccelerationProfile(self, num_pts=4, N=100):
		acc_pts = np.random.uniform(-0.2, 0.2, num_pts)
		#print(acc_pts)
	...
	def convert_state_to_int(self, state):
		...
		increments.reverse()
		increments = [self.num_ustates,] + increments
	...
	def ConvertCurrentMDPStateToDiscrete(self):
		...
		# print(disc_rel_dist, self.rel_dist_list[disc_rel_dist], disc_rel_vel, self.rel_vel_list[disc_rel_vel], disc_physical_state)
	...
	def step(self, action):
		...
		# print("--------------")
		# print(self.mdp_state, self.episode_steps)
