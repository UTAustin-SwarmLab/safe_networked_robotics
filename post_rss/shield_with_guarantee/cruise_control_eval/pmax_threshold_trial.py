...
for td in range(num_time_delays):
    ...
    for rel_dist_val in range(num_rel_dist_states):
        for rel_vel_val in range(num_rel_vel_states):
            ...
            req_states = [state for state in states if state[0] == physical_state]
            #print(req_states)
            req_state_values = [state_values[state] for state in req_states]
            val = min(req_state_values)
            print(val)
            # stay control vector
...
plt.savefig('constant_generated/pmax_plot_combined_max_safety.png')
